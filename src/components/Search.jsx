  ...
  const handleChange = (e) => {
    ...
    if (e.target.value === "") {
      ...
      let filterBlogs = props.posts.filter(
        (post) =>
          _.toString(post.data.title)
            .toLowerCase()
            .includes(inputVal().toLowerCase()) ||
          _.toString(post.data.description)
            .toLowerCase()
            .includes(inputVal().toLowerCase())
      );
      ...
      cloneBlogs.forEach((blog) => {
        ...
        if (blog.data.description) {
          blog.data.description = blog.data.description.replace(
            reg,
            (match) => {
              return `<span class="text-skin-active font-bold">${match}</span>`;
            }
    ...
    <div>
      ...
      <div class="my-4">
        {resultPosts().map((post) => (
          <>
            <a
              class="flex text-xl underline-offset-4 decoration-skin-base decoration-wavy hover:underline hover:decoration-sky-500 font-bold flex-wrap"
